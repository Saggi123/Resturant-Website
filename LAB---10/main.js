...
app.get("/view", (req, res) => {
    let qry = "select * from customerdetails1 ";
...
app.get("/addstudent", (req, res) => {
    ...
    // Sanitization XSS...
    let qry = "select * from customerdetails1 where emailid=? or phoneno=?";
    mysql.query(qry, [email, phone], (err, results) => {
        if (err)
            ...
            if (results.length > 0) {
                ...
                // insert query
                let qry2 = "insert into customerdetails1 values(?,?,?,?)";
...
app.get("/searchstudent", (req, res) => {
    ...
    let qry = "select * from customerdetails1 where phoneno=?";
...
app.get("/removestudent", (req, res) => {
    ...
    let qry = "delete from customerdetails1 where phoneno=?";
    mysql.query(qry, [phone], (err, results) => {
        if (err) throw err
        else {
            if (results.affectedRows > 0) {
